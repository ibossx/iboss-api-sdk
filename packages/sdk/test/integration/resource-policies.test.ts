...
describe("resource-policy patch UPDATE (DEVELOP-34914)", () => {
  it("get-merge-post preserves catN families when the agent sends only aiRiskEnabled", async () => {
    ...
    })) as ResourcePolicySettingsSummary;

    expect(summary.kind).toBe("resource");
    expect(summary.aiRisk.enabled).toBe(true);
    expect(summary.aiRisk.engines).toBe("chatgpt");
    expect(summary.groups.linkPolicyToAllSubjects).toBe(true);
  ...
  it("throws IbossVerifyError when POST succeeds but destinations do not persist", async () => {
    const state = createMockState();
    const client = makeClient(state);
    const created = await client.policies.createLayer({
      name: "AI Security Policy",
    ...
    await expect(
      client.policies.patchResourcePolicySettings(created.customCategoryId, {
...
describe("typed destinations (DEVELOP-34916)", () => {
  it("ensureAiSecurityDestination sets bit 110 and categoriesSelectedType 0", async () => {
    ...
    const summary = (await client.policies.ensureAiSecurityDestination(
    ...
    expect(summary.customType).toBe(13);
  ...
  it("createResourcePolicy returns verified GET settings, not just ids", async () => {
    ...
    expect(policy.aiRisk.enabled).toBe(true);
    expect(policy.dlpPolicyMethod).toBe(2);
